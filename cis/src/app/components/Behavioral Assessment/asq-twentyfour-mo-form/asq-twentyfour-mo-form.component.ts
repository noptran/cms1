...
import { AsqTwentyFour } from './asqTwentyfour';
import { MatDialog } from '@angular/material';
import { FormEditorComponent } from '../../form-editor/form-editor.component';
import { AlertDialogComponent } from '../../forms/alert-dialog/alert-dialog.component';
import { isNullOrUndefined } from 'util';
@Component({
  selector: 'asq-twentyfour-mo-form',
  templateUrl: './asq-twentyfour-mo-form.component.html',
  styleUrls: ['./asq-twentyfour-mo-form.component.scss', '../../forms/forms-common.scss', '../behavior.common.scss']
})
export class AsqTwentyfourMoFormComponent implements OnInit {
  asqtwentyFour: AsqTwentyFour = new AsqTwentyFour();
  @Input() model: any;
  docId: string;
  date: any;
  staffName: string;
  ...
  ngOnInit() {
    ...
    // if its a new form
    if (!this.docId) {
      this.model.name = Constants.PROGRAMS.BEHAVIORAL_ASSESSMENTS.FORMS.ASQ_24MO;
      this.model.program = Constants.PROGRAMS.BEHAVIORAL_ASSESSMENTS.SHORT_NAME;
      this.model.clientId = '';
      this.model.formName = this.model.name;
      this.model.totalQuestions = Constants.PROGRAMS.BEHAVIORAL_ASSESSMENTS.TOTAL_QUESTIONS_COUNT.ASQ_24MO;
      ...
      this.model.validationFields = ['CompletionDate', 'staffName', 'ClientName', 'DateOfBirth',
        ...
        'Question13Or', 'Question13Description', 'Question21Or', 'Question21Description', 'Question30Description',
        'Question31Description', 'ConcernsSleep', 'Question32Description', 'Worry',
        'Question33Description', 'Enjoy', 'Question34Description', 'Concerns1_16', 'ConcernsComments',
 'Question1', 'Question5', 'Question9', 'Question13', 'Question17', 'Question21', 'Question25',
  ...
  finalize() {
    ...
    if (this.model.disableValidate === true) {
      ...
      this.asqtwentyFour.valueArr.map((val) => {
        if (this.model[`${Object.keys(val)[0]}`].value === '') {
          value.push(Object.values(val)[0]);
        }
      });
      this.asqtwentyFour.optionIndex.map((val) => {
  ...
  otherSpecifyValidation() {
    if (!isNullOrUndefined(this.model.ConcernsSleep.fieldOptionIndex)) {
      if (this.model.ConcernsSleep.fieldOptionIndex === '0') {
        this.asqtwentyFour.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question32Description') {
            this.asqtwentyFour.valueArr.splice(index, 1);
          }
        });
        this.asqtwentyFour.valueArr.push({ Question32Description: 'Do you have concerns about your baby’s eating or sleeping behaviors? If yes please explain' });
      }  else {
        this.asqtwentyFour.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question32Description') {
            this.asqtwentyFour.valueArr.splice(index, 1);
    ...
    if (!isNullOrUndefined(this.model.Worry.fieldOptionIndex)) {
      if (this.model.Worry.fieldOptionIndex === '0') {
        ...
        this.asqtwentyFour.valueArr.push({ Question33Description: 'Does anything about your baby worry you? If yes, please explain' });
      } else {
        this.asqtwentyFour.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question33Description') {
            this.asqtwentyFour.valueArr.splice(index, 1);
    ...
    if (!isNullOrUndefined(this.model.Question13.fieldOptionIndex)) {
      if (this.model.Question13.fieldOptionIndex !== '0') {
        ...
        this.asqtwentyFour.valueArr.push({ Question13Or: 'Does your child have any eating problems, such as gagging, vomiting, or comment' });
        this.asqtwentyFour.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question13Description') {
            this.asqtwentyFour.valueArr.splice(index, 1);
          }
        });
        this.asqtwentyFour.valueArr.push({ Question13Description: 'Does your child have any eating problems, such as gagging, vomiting, or describe' });
      } else {
        this.asqtwentyFour.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question13Or') {
            this.asqtwentyFour.valueArr.splice(index, 1);
    ...
    if (!isNullOrUndefined(this.model.Question21.fieldOptionIndex)) {
      if (this.model.Question21.fieldOptionIndex !== '0') {
        ...
        this.asqtwentyFour.valueArr.push({ Question21Or: 'Does your child do things over and over and get upset when you try to stop him? For example, does he rock, flap his hands, spin, or comment' });
        this.asqtwentyFour.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question21Description') {
            this.asqtwentyFour.valueArr.splice(index, 1);
          }
        });
        this.asqtwentyFour.valueArr.push({ Question21Description: 'Does your child do things over and over and get upset when you try to stop him? For example, does he rock, flap his hands, spin, or describe' });
      } else {
        this.asqtwentyFour.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question21Or') {
            this.asqtwentyFour.valueArr.splice(index, 1);
    ...
    if (!isNullOrUndefined(this.model.Question30.fieldOptionIndex)) {
      if (this.model.Question30.fieldOptionIndex !== '0') {
        this.asqtwentyFour.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question30Description') {
            this.asqtwentyFour.valueArr.splice(index, 1);
          }
        });
        this.asqtwentyFour.valueArr.push({ Question30Description: 'Is your child too worried or fearful? If “sometimes” or “often or always,” please explain' });
      } else {
        this.asqtwentyFour.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question30Description') {
            this.asqtwentyFour.valueArr.splice(index, 1);
    ...
    if (!isNullOrUndefined(this.model.Question31.fieldOptionIndex)) {
      if (this.model.Question31.fieldOptionIndex !== '0') {
        ...
        this.asqtwentyFour.valueArr.push({ Question31Description: 'Has anyone shared concerns about your child’s behaviors? If “sometimes” or “often or always” please explain:' });
      } else {
        this.asqtwentyFour.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question31Description') {
            this.asqtwentyFour.valueArr.splice(index, 1);
    ...
    if (!isNullOrUndefined(this.model.Concerns1_16.fieldOptionIndex)) {
      if (this.model.Concerns1_16.fieldOptionIndex === '0') {
        ...
        this.asqtwentyFour.valueArr.push({ ConcernsComments: 'Any concerns marked on scored items Comments' });
      } else {
        this.asqtwentyFour.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'ConcernsComments') {
            this.asqtwentyFour.valueArr.splice(index, 1);
