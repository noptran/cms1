...
import { AsqFortyEight } from './asqFortyeight';
import { FormEditorComponent } from '../../form-editor/form-editor.component';
import { MatDialog } from '@angular/material';
import { AlertDialogComponent } from '../../forms/alert-dialog/alert-dialog.component';
import { isNullOrUndefined } from 'util';
@Component({
  selector: 'asq-fortyeight-form',
  templateUrl: './asq-fortyeight-form.component.html',
  styleUrls: ['./asq-fortyeight-form.component.scss', '../../forms/forms-common.scss', '../behavior.common.scss']
})
export class AsqFortyeightFormComponent implements OnInit {
  asqFortyEight: AsqFortyEight = new AsqFortyEight();
  @Input() model: any;
  docId: string;
  date: any;
  staffName: string;
  ...
  ngOnInit() {
    ...
    // if its a new form
    if (!this.docId) {
      this.model.name = Constants.PROGRAMS.BEHAVIORAL_ASSESSMENTS.FORMS.ASQ_48MO;
      this.model.program = Constants.PROGRAMS.BEHAVIORAL_ASSESSMENTS.SHORT_NAME;
      this.model.clientId = '';
      this.model.formName = this.model.name;
      this.model.totalQuestions = Constants.PROGRAMS.BEHAVIORAL_ASSESSMENTS.TOTAL_QUESTIONS_COUNT.ASQ_48MO;
      ...
      this.model.validationFields = ['CompletionDate', 'staffName',
        ...
        'Question11Or', 'Question11Description', 'Question22Or', 'Question22Description',
        'Question34Description', 'Question36Description', 'ConcernsSleep', 'Question37Description', 'Worry',
        'Question38Description', 'Enjoy',
        'Question39Description', 'Concerns1_16', 'ConcernsComments',
  ...
  finalize() {
    ...
    if (this.model.disableValidate === true) {
      ...
      this.asqFortyEight.valueArr.map((val) => {
        if (this.model[`${Object.keys(val)[0]}`].value === '') {
          value.push(Object.values(val)[0]);
        }
      });
      this.asqFortyEight.optionIndex.map((val) => {
  ...
  otherSpecifyValidation() {
    if (!isNullOrUndefined(this.model.ConcernsSleep.fieldOptionIndex)) {
      if (this.model.ConcernsSleep.fieldOptionIndex === '0') {
        this.asqFortyEight.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question37Description') {
            this.asqFortyEight.valueArr.splice(index, 1);
          }
        });
        this.asqFortyEight.valueArr.push({ Question37Description: 'Do you have concerns about your baby’s eating or sleeping behaviors? If yes please explain' });
      } else {
        this.asqFortyEight.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question37Description') {
            this.asqFortyEight.valueArr.splice(index, 1);
    ...
    if (!isNullOrUndefined(this.model.Worry.fieldOptionIndex)) {
      if (this.model.Worry.fieldOptionIndex === '0') {
        ...
        this.asqFortyEight.valueArr.push({ Question38Description: 'Does anything about your baby worry you? If yes, please explain' });
      } else {
        this.asqFortyEight.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question38Description') {
            this.asqFortyEight.valueArr.splice(index, 1);
    ...
    if (!isNullOrUndefined(this.model.Question11.fieldOptionIndex)) {
      if (this.model.Question11.fieldOptionIndex !== '0') {
        ...
        this.asqFortyEight.valueArr.push({ Question11Or: 'Does your child have eating problems? For example, does he stuff food, vomit, eat things that are not food comment' });
        this.asqFortyEight.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question11Description') {
            this.asqFortyEight.valueArr.splice(index, 1);
          }
        });
        this.asqFortyEight.valueArr.push({ Question11Description: 'Does your child have eating problems? For example, does he stuff food, vomit, eat things that are not food describe' });
      } else {
        this.asqFortyEight.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question11Or') {
            this.asqFortyEight.valueArr.splice(index, 1);
    ...
    if (!isNullOrUndefined(this.model.Question22.fieldOptionIndex)) {
      if (this.model.Question22.fieldOptionIndex !== '0') {
        ...
        this.asqFortyEight.valueArr.push({ Question22Or: 'Does your child do things over and over and get upset when you try to stop him? For example, does he rock, flap his hands, spin comment' });
        this.asqFortyEight.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question22Description') {
            this.asqFortyEight.valueArr.splice(index, 1);
          }
        });
        this.asqFortyEight.valueArr.push({ Question22Description: 'Does your child do things over and over and get upset when you try to stop him? For example, does he rock, flap his hands, spin describe' });
      } else {
        this.asqFortyEight.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question22Or') {
            this.asqFortyEight.valueArr.splice(index, 1);
    ...
    if (!isNullOrUndefined(this.model.Question34.fieldOptionIndex)) {
      if (this.model.Question34.fieldOptionIndex !== '2') {
        this.asqFortyEight.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question34Description') {
            this.asqFortyEight.valueArr.splice(index, 1);
          }
        });
        this.asqFortyEight.valueArr.push({ Question34Description: 'Is your child too worried or fearful? If “sometimes” or “often or always,” please explain' });
      } else {
        this.asqFortyEight.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question34Description') {
            this.asqFortyEight.valueArr.splice(index, 1);
    ...
    if (!isNullOrUndefined(this.model.Question36.fieldOptionIndex)) {
      if (this.model.Question36.fieldOptionIndex !== '0') {
        ...
        this.asqFortyEight.valueArr.push({ Question36Description: 'Has anyone shared concerns about your child’s behaviors? If “sometimes” or “often or always” please explain:' });
      } else {
        this.asqFortyEight.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'Question36Description') {
            this.asqFortyEight.valueArr.splice(index, 1);
    ...
    if (!isNullOrUndefined(this.model.Concerns1_16.fieldOptionIndex)) {
      if (this.model.Concerns1_16.fieldOptionIndex === '0') {
        ...
        this.asqFortyEight.valueArr.push({ ConcernsComments: 'Any concerns marked on scored items Comments' });
      } else {
        this.asqFortyEight.valueArr.map((val, index) => {
          if (Object.keys(val)[0] === 'ConcernsComments') {
            this.asqFortyEight.valueArr.splice(index, 1);
