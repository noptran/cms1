...
@Component({
  selector: 'app-csostaff',
  templateUrl: './csostaff.component.html',
  styleUrls: ['./csostaff.component.scss']
})
export class CsostaffComponent implements OnInit {

  personName = 'CSO Staff';
  customizedArray = 'customReport';
  columnToSorted = 'CSOStaffID';
  filter = 'CSOStaff';
  tableArray = 'cSOStaff';
  addLink = "/reports/csoStaff/new";
  navigateTo = '/reports/csoStaff/details';
  isSearchAny = true;
  breadcrumbs = [
  ...
  add() {
    this.router.navigate(['/reports/csoStaff/new']);
  }
  ...
  getPerson(initial: any, end: any) {
    ...
    this.personData['filter'] = 'CSOStaff';
    ...
    this.clildFormService.getPerson(this.personData).then(data => {
      ...
      this.reportsViewList = data.cSOStaff;
      this.numericColumns = data.dataTypes;
      this.rowData = data['cSOStaff'] ? data['cSOStaff'] : data['customReport'];
  ...
  onRowSelected(event) {
    ...
    this.router.navigate(['/reports/csoStaff/details']);
  }
