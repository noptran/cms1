...
import { CisAsq36Mo } from './cis-asq-36-mo';
...
@Component({
  selector: 'app-asq-36-mo',
  templateUrl: './asq-36-mo.component.html',
  styleUrls: ['./asq-36-mo.component.scss', '../../family-preservation/family-preservation.scss'],
  outputs: ['asqThirtySixOut', 'cancelOut'],
})
export class Asq36MoComponent implements OnInit {
  asqThirtySixMoForm: FormGroup;
  unableToAccesForm: FormGroup;
  cisAsqThirtySixMo: CisAsq36Mo = new CisAsq36Mo();
  ...
  @Output()
  asqThirtySixOut = new EventEmitter();
  ...
  formValidation() {
    this.asqThirtySixMoForm = this._fb.group({
      name: [null],
      program: [null],
  ...
  calculate() {
    ...
    this.cisAsqThirtySixMo.TotalScore.value = 0;
  ...
  asqTotalScoreCalculation() {
    ...
    for (let i = 1; i <= this.cisAsqThirtySixMo.totalQuestions; i++) {
      this.cisAsqThirtySixMo["Question" + i + "Sum"].value = 0;
      if (this.cisAsqThirtySixMo["Question" + i].fieldOptionIndex === "0") {
        this.cisAsqThirtySixMo["Question" + i + "Sum"].value = 0
      }
      if (this.cisAsqThirtySixMo["Question" + i].fieldOptionIndex === "1") {
        this.cisAsqThirtySixMo["Question" + i + "Sum"].value = 5
      }
      if (this.cisAsqThirtySixMo["Question" + i].fieldOptionIndex === "2") {
        this.cisAsqThirtySixMo["Question" + i + "Sum"].value = 10
      }
      if ((this.cisAsqThirtySixMo["Question" + i + "Concern"].fieldOptionIndex === true) || this.cisAsqThirtySixMo["Question" + i + "Concern"].fieldOptionIndex === "1") {
        this.cisAsqThirtySixMo["Question" + i + "Concern"].fieldOptionIndex = "1";
        this.cisAsqThirtySixMo["Question" + i + "Sum"].value += 5;
      }
      this.cisAsqThirtySixMo.TotalScore.value = parseInt(this.cisAsqThirtySixMo.TotalScore.value) + this.cisAsqThirtySixMo["Question" + i + "Sum"].value;
    }
  }

  asqScoreChartCalculation() {
    if (this.cisAsqThirtySixMo.TotalScore.value < this.cisAsqThirtySixMo.lowRiskValue) {
      this.cisAsqThirtySixMo.babyTotalScore.fieldOptionIndex = "0";
    } else if ((this.cisAsqThirtySixMo.TotalScore.value > this.cisAsqThirtySixMo.lowRiskValue) && (this.cisAsqThirtySixMo.TotalScore.value < this.cisAsqThirtySixMo.monitorValue)) {
      this.cisAsqThirtySixMo.babyTotalScore.fieldOptionIndex = "1";
    } else if (this.cisAsqThirtySixMo.TotalScore.value > this.cisAsqThirtySixMo.lowRiskValue) {
      this.cisAsqThirtySixMo.babyTotalScore.fieldOptionIndex = "2";
    }
  }
  ...
  async saveForm(source: any) {
    ...
    if (this.cisFormJson.cmsCisPdfDoc[this.currentCisVersion].isFinalized) {
      ...
      this.asqThirtySixMoForm.disable();
    }
    else {
  ...
  getJson() {
    ...
    this.formJsonValue = this._openCards.getBehavioralCisJson(data)
      .then(async (data) => {
        ...
        this.cisAsqThirtySixMo = await JSON.parse(data.cmsCisPdfDoc[this.currentCisVersion].pdfJsonData);
        if (this.cisFormJson.cmsCisPdfDoc[this.currentCisVersion].isFinalized) {
  ...
  editForm() {
    if (parseInt(this.currentCisVersion) == this.cisFormJson.cmsCisPdfDoc.length - 1) {
      ...
      if (this.cisAsqThirtySixMo.disableValidate === true) {
        ...
        this.asqThirtySixMoForm.enable();
        this.unableToAccesForm.enable();
      }
  ...
  handleDataField(event) {
    ...
    if (event === true) {
      ...
      this.asqThirtySixMoForm.reset();

      this.cisAsqThirtySixMo.disableValidate = true;
      this.asqThirtySixMoForm.disable();


    }
    else {
      this.cisAsqThirtySixMo.disableValidate = false;
      this.cisAsqThirtySixMo.remarks = "";
      this.asqThirtySixMoForm.enable();
    }
  ...
  saveRemarks(remarks) {
    // this.cisAsqThirtySixMo.remarks.value = remarks.value;
  }
