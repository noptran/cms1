...
import { CisAsq48Mo } from './cis-asq-48-mo';
...
@Component({
  selector: 'app-asq-48-mo',
  templateUrl: './asq-48-mo.component.html',
  styleUrls: ['./asq-48-mo.component.scss', '../../family-preservation/family-preservation.scss'],
  outputs: ['asqFourtEightOut', 'cancelOut'],
})
export class Asq48MoComponent implements OnInit {
  unableToAccesForm: FormGroup;
  asqFourtyEightMoForm: FormGroup;
  cisAsqFourtyEightMo: CisAsq48Mo = new CisAsq48Mo();
  ...
  @Output()
  asqFourtEightOut = new EventEmitter();
  ...
  formValidation() {
    this.asqFourtyEightMoForm = this._fb.group({
      name: [null],
      program: [null],
  ...
  calculate() {
    ...
    this.cisAsqFourtyEightMo.TotalScore.value = 0;
  ...
  asqTotalScoreCalculation() {
    ...
    for (let i = 1; i <= this.cisAsqFourtyEightMo.totalQuestions; i++) {
      this.cisAsqFourtyEightMo["Question" + i + "Sum"].value = 0;
      if (this.cisAsqFourtyEightMo["Question" + i].fieldOptionIndex === "0") {
        this.cisAsqFourtyEightMo["Question" + i + "Sum"].value = 0
      }
      if (this.cisAsqFourtyEightMo["Question" + i].fieldOptionIndex === "1") {
        this.cisAsqFourtyEightMo["Question" + i + "Sum"].value = 5
      }
      if (this.cisAsqFourtyEightMo["Question" + i].fieldOptionIndex === "2") {
        this.cisAsqFourtyEightMo["Question" + i + "Sum"].value = 10
      }
      if ((this.cisAsqFourtyEightMo["Question" + i + "Concern"].fieldOptionIndex === true) || this.cisAsqFourtyEightMo["Question" + i + "Concern"].fieldOptionIndex === "1") {
        this.cisAsqFourtyEightMo["Question" + i + "Concern"].fieldOptionIndex = "1";
        this.cisAsqFourtyEightMo["Question" + i + "Sum"].value += 5;
      }
      this.cisAsqFourtyEightMo.TotalScore.value = parseInt(this.cisAsqFourtyEightMo.TotalScore.value) + this.cisAsqFourtyEightMo["Question" + i + "Sum"].value;
    }
  }

  asqScoreChartCalculation() {
    if (this.cisAsqFourtyEightMo.TotalScore.value < this.cisAsqFourtyEightMo.lowRiskValue) {
      this.cisAsqFourtyEightMo.babyTotalScore.fieldOptionIndex = "0";
    } else if ((this.cisAsqFourtyEightMo.TotalScore.value > this.cisAsqFourtyEightMo.lowRiskValue) && (this.cisAsqFourtyEightMo.TotalScore.value < this.cisAsqFourtyEightMo.monitorValue)) {
      this.cisAsqFourtyEightMo.babyTotalScore.fieldOptionIndex = "1";
    } else if (this.cisAsqFourtyEightMo.TotalScore.value > this.cisAsqFourtyEightMo.lowRiskValue) {
      this.cisAsqFourtyEightMo.babyTotalScore.fieldOptionIndex = "2";
    }
  }
  ...
  async saveForm(source: any) {
    ...
    if (this.cisFormJson.cmsCisPdfDoc[this.currentCisVersion].isFinalized) {
      ...
      this.asqFourtyEightMoForm.disable();
    }
    else {
  ...
  getJson() {
    ...
    this.formJsonValue = this._openCards.getBehavioralCisJson(data)
      .then(async (data) => {
        ...
        this.cisAsqFourtyEightMo = await JSON.parse(data.cmsCisPdfDoc[this.currentCisVersion].pdfJsonData);
        if (this.cisFormJson.cmsCisPdfDoc[this.currentCisVersion].isFinalized) {
  ...
  editForm() {
    if (parseInt(this.currentCisVersion) == this.cisFormJson.cmsCisPdfDoc.length - 1) {
      ...
      if (this.cisAsqFourtyEightMo.disableValidate === true) {
        ...
        this.asqFourtyEightMoForm.enable();
        this.unableToAccesForm.enable();
      }
  ...
  handleDataField(event) {
    ...
    if (event === true) {
      ...
      this.asqFourtyEightMoForm.reset();

      this.cisAsqFourtyEightMo.disableValidate = true;
      this.asqFourtyEightMoForm.disable();


    }
    else {
      this.cisAsqFourtyEightMo.disableValidate = false;
      this.cisAsqFourtyEightMo.remarks = "";
      this.asqFourtyEightMoForm.enable();
    }
  ...
  saveRemarks(remarks) {
    // this.cisAsqFourtyEightMo.remarks.value = remarks.value;
  }
