...
export class PlacementPlanBlankComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.currentURL == '/placement-plan-blank(print:print/placement-plan-blank)') {
      this.isBlankForm = true;
    }
    else if (this.currentURL == '/placement-plan-draft(print:print/placement-plan-blank)') {
  ...
  async getRespiteAuthorization() {
    let placementReq = { placementID: this.currentPlacementId }
    await this._placement.getRespiteAuthorization(placementReq).then((data: any) => {
