...
import { KinshipAp } from './kinship-ap';
import { isNullOrUndefined } from 'util';
import { ActivatedRoute } from '@angular/router';
import * as moment from 'moment';

@Component({
  selector: 'app-placement-agreement-kinship-ap',
  templateUrl: './placement-agreement-kinship-ap.component.html',
  styleUrls: ['./placement-agreement-kinship-ap.component.scss']
})
export class PlacementAgreementKinshipAPComponent implements OnInit {
  placementData: KinshipAp = new KinshipAp();
  ...
  getData() {
    ...
    const placementReq = { placementID: this._placement.getSavedPlacementId() || this.currentPlacementID };
  ...
  getAuthData() {
    ...
    if (this._placement.getSavedPlacementId() || this.currentPlaementDetailID) {
      ...
      this._placement.getPlacementAuthorization(placementReq).then((data: any) => {
        ...
        const authInfo = (data.placementAuthorization.length > 0) ? data.placementAuthorization[data.placementAuthorization.length - 1] : null;
