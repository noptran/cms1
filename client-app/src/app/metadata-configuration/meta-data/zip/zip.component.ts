...
export class ZipComponent implements OnInit {
  ...
  ngOnInit() {this.getListofZipcodes();}
  
   /**
   * 
   * @returns the list view zipcodes
   * 
   * @memberOf CsoOfficeComponent
   */
  getListofZipcodes(){
    let req = {Object:'zipcode',beginPagination:this.initial,endPagination:this.end}
    ...
    this._meta.getListofMetaData(req).then(data=>{
      ...
      if(data.dropDownObject.length > 0){
        this.zipList.push(data.dropDownObject)
      }
      return this._meta.generateMetaListView(this.zipList).then(data=>{
        this.rowData = data[0];
